...
SELECT
    ...
    -- 城市备注
    t7.city_num,
    CONCAT(STRING((CAST((SPLIT(city_num, '-')[1] / area_num) AS DECIMAL(13, 2))) * 100), '%')   AS city_1,
    t7.city_num_2,
    CONCAT(STRING((CAST((SPLIT(city_num_2, '-')[1] / area_num) AS DECIMAL(13, 2))) * 100), '%') AS city_2,
    CONCAT(STRING((CAST(
            ((area_num - SPLIT(city_num, '-')[1] - SPLIT(city_num_2, '-')[1]) / area_num) AS DECIMAL(13, 2))) * 100),
           '%')                                                                                 AS city_o
...
-- 地区点击商品排序
         ...
         FROM (
                  ...
                  FROM (
                           -- 城市点击商品排序
                           SELECT *,
                                  ROW_NUMBER() OVER (PARTITION BY t1.city_id ORDER BY num DESC ) AS city_rank
                           FROM (
                                    -- 城市点击商品数量
                                    SELECT
                                        city_id,
...
JOIN (
         ...
         ) t7 ON t7.click_product_id = t5.click_product_id AND t7.area = t5.area AND t7.city_num_2 IS NOT NULL
